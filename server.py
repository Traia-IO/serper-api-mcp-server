#!/usr/bin/env python3
"""
Serper API MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('serper-api_mcp')
...
logger.info("="*80)
logger.info(f"Serper API MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("Serper API MCP Server", host="0.0.0.0")
...
# - SERPER_API_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="serper-api-mcp-server"  # MCP server ID for tracking
    )
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Serper API MCP Server")
    logger.info("="*80)
