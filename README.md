...
This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Serper API API. It enables AI agents and LLMs to interact with Serper API through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Serper API endpoints
...
- **Serper API Website**: [https://google.serper.dev](https://google.serper.dev)
...
*Note: Replace `example_tool` with actual Serper API API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/serper-api-mcp-server.git
   cd serper-api-mcp-server
   ```
...
Clients with their own Serper API API key can use the server for free:
...
**Flow**:
1. Client provides their Serper API API key
2. Server uses client's API key to call Serper API API
3. No payment required
4. Client pays Serper API directly
...
6. Server uses its INTERNAL Serper API API key to call the API
...
```bash
# Required
SERPER_API_KEY=your_internal_serper-api_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Serper API API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Serper API endpoints
...
- `SERPER_API_KEY`: Your Serper API API key (required)
## Troubleshooting
